...
def harris(image, block_size, kernel_size):
    ...
    block_size : int
        Neighborhood size for eigen value computation
    kernel_size : int
        Size of Sobel kernel. Must be 1, 3, 5 or 7
        
    Returns
    ------------------
    H : (M, N) ndarray (dtype float32)
        The Harris response, with negative values removed
    """
...
def cut_out_rois(image, col_indices, row_indices, roi_size):
    ...
    roi_size : int
        Size of the ROI. Must be odd.
    
    Returns
    -----------------
    rois : list
        List of region of interests (roi_size x roi_size arrays)
